...
function setDate(): void {
  ...
  const formattedDate: string = `${date.getFullYear()}-${Number(
    date.getMonth() + 1
  )
    .toString()
    .padStart(2, "0")}-${date.getDate().toString().padStart(2, "0")}`;
  const transDateInput = document.getElementById(
    "transDate"
  ) as HTMLInputElement;
  if (transDateInput) {
...
function loadCategories(): void {
  fetch(`${host}/categories`)
    ...
    .then((data: { name: string; color: string; type: string }[]) => {
      categories = data;
      const transCategoryElement = document.getElementById(
        "transCategory"
...
function loadTranSHistory(): void {
  ...
  fetch(`${host}/transactions`)
    ...
    .then((data: any[]) => {
      data = [...data].reverse();
      const transHistoryElement = document.getElementById(
        "transHistory"
      ) as HTMLElement;
      ...
      (document.getElementById("income") as HTMLElement).innerHTML =
        income.toFixed(2);
      (document.getElementById("expense") as HTMLElement).innerHTML =
        expense.toFixed(2);
      (document.getElementById("balance") as HTMLElement).innerHTML = (
        income - expense
      ).toFixed(2);
    });
}
...
function addTrans(): void {
  ...
  const category: string = (
    document.getElementById("transCategory") as HTMLSelectElement
  ).value;
  const categoryType = categories.find((cate) => cate.name === category)?.type;
  const type: string = categoryType === "income" ? "Income" : "Expense";
  const date: string = (
    document.getElementById("transDate") as HTMLInputElement
  ).value;
  const description: string = (
    document.getElementById("transDescription") as HTMLTextAreaElement
  ).value;
  const categoryColor: string | undefined = categories.find(
  ...
  transHistoryElement.innerHTML =
    `
            <tr>
                ...
                }'>${type.toLowerCase() === "income" ? "+" : "-"}$${amount}</td>
            </tr>` + transHistoryElement.innerHTML;

  if (type === "Expense") {
    (document.getElementById("expense") as HTMLElement).innerHTML = (
      parseInt((document.getElementById("expense") as HTMLElement).innerHTML) +
      amount
    ).toString();
    (document.getElementById("balance") as HTMLElement).innerHTML = (
      parseInt((document.getElementById("balance") as HTMLElement).innerHTML) -
      amount
    ).toString();
  } else if (type === "Income") {
    (document.getElementById("income") as HTMLElement).innerHTML = (
      parseInt((document.getElementById("income") as HTMLElement).innerHTML) +
      amount
    ).toString();
    (document.getElementById("balance") as HTMLElement).innerHTML = (
      parseInt((document.getElementById("balance") as HTMLElement).innerHTML) +
